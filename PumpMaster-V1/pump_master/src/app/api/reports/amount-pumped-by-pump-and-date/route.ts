...
export async function GET(request: Request) {
	try {
		...
		const response = await fetch(
			`http://localhost:3001/reports/amount-pumped-by-pump-and-date?startDate=${formattedStart}&endDate=${formattedEnd}`,
			{
		...
		console.error("Error fetching amount pumped by pump and date:", error);
		return NextResponse.json(
			{ error: "Failed to fetch amount pumped by pump and date data" },
			{ status: 500 },
		);
	}
}
